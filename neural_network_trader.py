"""
Rete Neurale Multi-Timeframe per Trading
Implementazione Python della rete neurale TradingView (Delfini, Squali, Balene)

Architettura: 15 -> 30 -> 9 -> 1 neuroni
Funzioni attivazione: Linear (input), Tanh (hidden/output)
"""
...
class NeuralNetworkTrader:
    """
    Rete neurale pre-addestrata per identificare trend e punti di ingresso/uscita
    """

    def __init__(self):
        """Inizializza la rete con i pesi pre-addestrati"""

        # Pesi Layer 0 -> Layer 1 (15 input -> 30 hidden)
        self.weights_l0_l1 = np.array([
        ...
        # Pesi Layer 1 -> Layer 2 (30 hidden -> 9 hidden)
        self.weights_l1_l2 = np.array([
            ...
            [0.2215807411, -0.5628295071, -0.8795982905, 0.9101585104, -1.0176831976, -0.0728884401, 0.6676331658, -0.7342174108, 0.00094428, 0.6439774272, -0.0345236026, 0.5830977027, -0.4058921837, -0.3991888077, -1.0090426973, -0.9324780698, -0.0888749165, 0.2466351736, 0.4993304601, -1.115408696, 0.9914246705, 0.9687743445, 0.1117130875, 0.7825109733, 0.2217023612, 0.3081256411, -0.1778007966, -0.3333287743, 1.0156352461, -0.1456257813],
        ...
        # Pesi Layer 2 -> Layer 3 (9 hidden -> 1 output)
        self.weights_l2_l3 = np.array([
    ...
    @staticmethod
    def linear(x):
        """Funzione di attivazione lineare (identity)"""
        return x

    def forward(self, price_change_pct: float) -> float:
        """
        Esegue forward pass attraverso la rete

        Args:
            price_change_pct: Variazione percentuale del prezzo (es: 0.02 per +2%)

        Returns:
            Output della rete (valore tra -1 e 1)
        """
        # Layer 0: Input (15 neuroni, tutti ricevono lo stesso valore)
        l0 = np.array([self.linear(price_change_pct)] * 15)

        # Layer 1: Hidden (30 neuroni con tanh)
        l1 = self.tanh(np.dot(self.weights_l0_l1, l0))

        # Layer 2: Hidden (9 neuroni con tanh)
        l2 = self.tanh(np.dot(self.weights_l1_l2, l1))

        # Layer 3: Output (1 neurone con tanh)
        l3 = self.tanh(np.dot(self.weights_l2_l3, l2))

        return l3

    def analyze_dataframe(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Analizza un DataFrame con dati OHLC

        Args:
            df: DataFrame con colonne: open, high, low, close

        Returns:
            DataFrame con colonna aggiuntiva 'nn_output'
        """
        df = df.copy()

        # Calcola ohlc4 (media di OHLC)
        df['ohlc4'] = (df['open'] + df['high'] + df['low'] + df['close']) / 4

        # Calcola la variazione percentuale
        df['price_change'] = df['ohlc4'].pct_change()

        # Applica la rete neurale
        df['nn_output'] = df['price_change'].apply(
            lambda x: self.forward(x) if pd.notna(x) else 0
        )

        return df


class MultiTimeframeNNTrader:
    """
    Sistema multi-timeframe con 3 reti neurali (Delfini, Squali, Balene)
    """

    def __init__(self):
        # 3 istanze della stessa rete per 3 timeframe diversi
        self.nn_dolphins = NeuralNetworkTrader()  # Delfini (timeframe corto)
        self.nn_sharks = NeuralNetworkTrader()    # Squali (timeframe medio)
        self.nn_whales = NeuralNetworkTrader()    # Balene (timeframe lungo)
    ...
    def analyze_multi_timeframe(
        ...
        """
        Analizza un simbolo su 3 timeframe diversi

        Args:
            symbol: Coppia di trading (es: 'BTCUSDT')
            tf_dolphins: Timeframe per Delfini (default: 15m)
            tf_sharks: Timeframe per Squali (default: 1h)
            tf_whales: Timeframe per Balene (default: 4h)
        ...
        # Analizza Delfini (timeframe corto)
        print(f"\n🐬 DELFINI - Timeframe: {tf_dolphins}")
        df_dolphins = loader.get_historical_klines(symbol, tf_dolphins, days_back)
        df_dolphins = self.nn_dolphins.analyze_dataframe(df_dolphins)
        results['dolphins'] = df_dolphins
        print(f"   Output medio: {df_dolphins['nn_output'].mean():.4f}")
        print(f"   Output ultimo: {df_dolphins['nn_output'].iloc[-1]:.4f}")

        # Analizza Squali (timeframe medio)
        print(f"\n🦈 SQUALI - Timeframe: {tf_sharks}")
        df_sharks = loader.get_historical_klines(symbol, tf_sharks, days_back)
        df_sharks = self.nn_sharks.analyze_dataframe(df_sharks)
        results['sharks'] = df_sharks
        print(f"   Output medio: {df_sharks['nn_output'].mean():.4f}")
        print(f"   Output ultimo: {df_sharks['nn_output'].iloc[-1]:.4f}")

        # Analizza Balene (timeframe lungo)
        print(f"\n🐋 BALENE - Timeframe: {tf_whales}")
        df_whales = loader.get_historical_klines(symbol, tf_whales, days_back)
        df_whales = self.nn_whales.analyze_dataframe(df_whales)
        results['whales'] = df_whales
        print(f"   Output medio: {df_whales['nn_output'].mean():.4f}")
        print(f"   Output ultimo: {df_whales['nn_output'].iloc[-1]:.4f}")
    ...
    def generate_signals(
        ...
        """
        Genera segnali di trading basati sui 3 timeframe

        Args:
            results: Risultati dell'analisi multi-timeframe
            lookback: Periodi da guardare indietro

        Returns:
            Dict con segnali e dettagli
        """
        # Ottieni gli ultimi valori
        nn_dolphins = results['dolphins']['nn_output'].iloc[-1]
        nn_sharks = results['sharks']['nn_output'].iloc[-1]
        nn_whales = results['whales']['nn_output'].iloc[-1]

        # Combina i 3 output
        combined = nn_dolphins + nn_sharks + nn_whales

        # Ottieni il prezzo corrente
        current_price = results['dolphins']['close'].iloc[-1]
        open_price = results['dolphins']['open'].iloc[-1]

        # Calcola get1 = open * -(combined)
        get1_values = []
        min_len = min(
            len(results['dolphins']),
            len(results['sharks']),
            len(results['whales'])
        )

        for i in range(min_len):
            nn_d = results['dolphins']['nn_output'].iloc[i]
            nn_s = results['sharks']['nn_output'].iloc[i]
            nn_w = results['whales']['nn_output'].iloc[i]
            open_p = results['dolphins']['open'].iloc[i]

            get1 = open_p * -(nn_d + nn_s + nn_w)
            get1_values.append(get1)

        # Prendi gli ultimi lookback valori
        recent_get1 = get1_values[-lookback:] if len(get1_values) >= lookback else get1_values

        highest = max(recent_get1)
        lowest = min(recent_get1)
        max_abs = max(highest, -lowest)

        # Normalizza
        normalized_signal = get1_values[-1] / max_abs if max_abs != 0 else 0
        ...
        # Identifica punti di ingresso/uscita basati su crossover
        df_dolphins = results['dolphins']

        # Cerca crossover (quando nn_output attraversa lo zero)
        crossovers = []
        for i in range(1, len(df_dolphins)):
            prev_val = df_dolphins['nn_output'].iloc[i-1]
            curr_val = df_dolphins['nn_output'].iloc[i]

            # Crossover al rialzo (da negativo a positivo)
            if prev_val < 0 and curr_val > 0:
                crossovers.append({
                    'type': 'BULLISH_CROSS',
                    'index': i,
                    'price': df_dolphins['close'].iloc[i],
                    'timestamp': df_dolphins.index[i]
                })

            # Crossover al ribasso (da positivo a negativo)
        ...
        # Identifica se siamo vicini a un crossover
        bars_since_last_cross = None
        entry_exit_signal = None

        if crossovers:
            last_cross = crossovers[-1]
            bars_since_last_cross = len(df_dolphins) - 1 - last_cross['index']

            # Segnali di ingresso/uscita se il crossover è recente (ultimi 10 bar)
            if bars_since_last_cross <= 10:
                if last_cross['type'] == 'BULLISH_CROSS':
                    entry_exit_signal = f"🟢 INGRESSO LONG - Crossover rialzista {bars_since_last_cross} bar fa"
                else:
                    entry_exit_signal = f"🔴 USCITA/SHORT - Crossover ribassista {bars_since_last_cross} bar fa"

        return {
            'trend': trend,
            'action': action,
            'combined_output': combined,
            'nn_dolphins': nn_dolphins,
            'nn_sharks': nn_sharks,
            'nn_whales': nn_whales,
    ...
    def print_analysis_report(self, symbol: str, signals: Dict):
        """Stampa report dettagliato dell'analisi"""

        print("\n" + "=" * 80)
        print(f"ANALISI RETE NEURALE MULTI-TIMEFRAME - {symbol}")
        print("=" * 80)

        print(f"\n📊 PREZZO CORRENTE: ${signals['current_price']:,.2f}")

        print(f"\n🎯 TREND IDENTIFICATO: {signals['trend']}")
        print(f"⚡ AZIONE RACCOMANDATA: {signals['action']}")

        print(f"\n🧠 OUTPUT RETI NEURALI:")
        print(f"   🐬 Delfini (breve):  {signals['nn_dolphins']:+.4f}")
        print(f"   🦈 Squali (medio):   {signals['nn_sharks']:+.4f}")
        print(f"   🐋 Balene (lungo):   {signals['nn_whales']:+.4f}")
        print(f"   📊 Combinato:        {signals['combined_output']:+.4f}")

        print(f"\n📈 SEGNALE NORMALIZZATO: {signals['normalized_signal']:+.4f}")

        if signals['entry_exit_signal']:
            print(f"\n{signals['entry_exit_signal']}")

        if signals['total_crossovers'] > 0:
            print(f"\n🔄 CROSSOVER RILEVATI: {signals['total_crossovers']}")

            # Mostra ultimi 3 crossover
            recent_crosses = signals['crossovers'][-3:]
            for cross in reversed(recent_crosses):
                cross_type_emoji = "🟢" if cross['type'] == 'BULLISH_CROSS' else "🔴"
                cross_type_name = "Rialzista" if cross['type'] == 'BULLISH_CROSS' else "Ribassista"
                print(f"   {cross_type_emoji} {cross_type_name}: ${cross['price']:,.2f} - {cross['timestamp']}")

        # Interpretazione
        print(f"\n💡 INTERPRETAZIONE:")
        if signals['action'] == 'BUY':
            print("   ✅ Le 3 reti neurali indicano momentum rialzista")
            print("   ✅ Considerare ingresso in posizione LONG")
        elif signals['action'] == 'SELL':
            print("   ⚠️  Le 3 reti neurali indicano momentum ribassista")
            print("   ⚠️  Considerare uscita o posizione SHORT")
        else:
            print("   ⏸️  Mercato laterale o incerto")
            print("   ⏸️  Attendere segnale più chiaro")
...
if __name__ == '__main__':
    print("=" * 80)
    print("TEST: Rete Neurale Multi-Timeframe")
    print("=" * 80)

    # Test singola rete
    print("\n--- Test Singola Rete Neurale ---")
    nn = NeuralNetworkTrader()

    test_changes = [0.01, -0.01, 0.02, -0.02, 0.00]
    for change in test_changes:
        output = nn.forward(change)
        print(f"Price change: {change:+.2%} → NN output: {output:+.4f}")
    ...
    # Genera segnali
    signals = trader.generate_signals(results, lookback=30)
